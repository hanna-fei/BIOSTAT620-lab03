...
### COMPLETE CODE HERE
p1 <- ggplot(cv_states, aes(x = date, y = new_cases)) + geom_point()
ggplotly(p1)

p2 <- ggplot(cv_states, aes(x = date, y = new_deaths)) + geom_point()
ggplotly(p2)
...
cv_states$per100k <- as.numeric(format(round(cv_states$cases/(cv_states$cases/100000),1),nsmall=1))
cv_states$newper100k <- as.numeric(format(round(cv_states$new_cases/(cv_states$new_cases/100000),1),nsmall=1))
cv_states$deathsper100k <- as.numeric(format(round(cv_states$deaths/(cv_states$deaths/100000),1),nsmall=1))
cv_states$newdeathsper100k <- as.numeric(format(round(cv_states$new_deaths/(cv_states$new_deaths/100000),1),nsmall=1))
...
```{r}
### FINISH CODE HERE
p <- ggplot(cv_states_today, aes(x=deathsper100k, y=pop_density, size=population)) + geom_point() + geom_smooth()
ggplotly(p)
```
...
# Create hover text to include state name, new cases per 100k, cases, and deaths
cv_per100$hover <- with(cv_per100, paste(state_name, 
                                         "<br>Cases per 100k:", newper100k,
